...
//         assertEq(
//             abi.encodePacked(uint8(MessagesLib.Call.UpdateMember), poolId, trancheId, member, validUntil),
// expectedHex
//         );
...
//         assertEq(
//             abi.encodePacked(uint8(MessagesLib.Call.IncreaseInvestOrder), poolId, trancheId, investor, currency,
// amount),
//             expectedHex
//         );
...
//         assertEq(
//             abi.encodePacked(uint8(MessagesLib.Call.IncreaseRedeemOrder), poolId, trancheId, investor, currency,
// amount),
//             expectedHex
//         );
...
//         assertEq(abi.encodePacked(uint8(MessagesLib.Call.Freeze), poolId, trancheId, investor.toBytes32()),
// expectedHex);

//         (uint64 decodedPoolId, bytes16 decodedTrancheId, address decodedInvestor) =
// MessagesLib.parseFreeze(expectedHex);
//         assertEq(uint256(decodedPoolId), poolId);
//         assertEq(decodedTrancheId, trancheId);
//         assertEq(decodedInvestor, investor);
//     }
...
//         assertEq(abi.encodePacked(uint8(MessagesLib.Call.DisallowInvestmentCurrency), poolId, currency),
// expectedHex);
