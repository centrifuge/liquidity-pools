...
contract ERC7540Vault is Auth, IERC7540 {
    ...
    function requestDeposit(
        ...
        validateOwner(owner);
        require(
            IERC20(asset).balanceOf(owner) >= assets,
            "ERC7540Vault/insufficient-balance"
        );

        require(
            manager.requestDeposit(address(this), assets, receiver, owner),
        ...
        require(
            data.length == 0 ||
                ...
                IERC7540DepositReceiver(receiver).onERC7540DepositReceived(
                    msg.sender,
                    owner,
                    REQUEST_ID,
                    assets,
    ...
    function requestRedeem(
        ...
        require(
            IERC20(share).balanceOf(owner) >= shares,
            "ERC7540Vault/insufficient-balance"
        );
        require(
            manager.requestRedeem(address(this), shares, receiver, owner),
        ...
        require(
            data.length == 0 ||
                receiver.code.length == 0 ||
                IERC7540RedeemReceiver(receiver).onERC7540RedeemReceived(
                    msg.sender,
                    owner,
                    REQUEST_ID,
                    shares,
    ...
    function claimableCancelDepositRequest(
        ...
        claimableAssets = manager.claimableCancelDepositRequest(
            address(this),
            owner
        );
    }
    ...
    function claimCancelDepositRequest(
        ...
        validateOwner(owner);
        assets = manager.claimCancelDepositRequest(
            address(this),
            receiver,
            owner
        );
        emit CancelDepositClaim(
            receiver,
            owner,
            REQUEST_ID,
            msg.sender,
    ...
    function claimableCancelRedeemRequest(
        ...
        claimableShares = manager.claimableCancelRedeemRequest(
            address(this),
            owner
        );
    }
    ...
    function claimCancelRedeemRequest(
        ...
        validateOwner(owner);
        shares = manager.claimCancelRedeemRequest(
            address(this),
            receiver,
            owner
        );
    ...
    function emitCancelDepositClaimable(
        address owner,
        uint256 assets
    ) public auth {
    ...
    function emitCancelRedeemClaimable(
        address owner,
        uint256 shares
    ) public auth {
