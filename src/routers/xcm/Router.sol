...
interface ConnectorLike {
  ...
  function addTranche(uint64 poolId, bytes16 trancheId, string memory tokenName, string memory tokenSymbol, uint256 price) external;
...
contract ConnectorXCMRouter {
    ...
    function handle(
        ...
        if (ConnectorMessages.isAddPool(_msg)) {
            ...
            (uint64 poolId, bytes16 trancheId, string memory tokenName, string memory tokenSymbol, uint256 price) = ConnectorMessages.parseAddTranche(_msg);
