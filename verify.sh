...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 {vault|trancheToken} contract_address"
    exit 1
fi
...
if [ "$type" == "trancheToken" ]; then
    decimals=$(cast call $contract_address 'decimals()(uint8)' --rpc-url $RPC_URL)
    echo "decimals: $decimals"
    forge verify-contract --constructor-args $(cast abi-encode "constructor(uint8)" $decimals) --watch --etherscan-api-key $ETHERSCAN_KEY $contract_address src/token/TrancheToken01.sol:TrancheToken --verifier-url $VERIFIER_URL --chain $CHAIN_ID
    ...
    echo "Error: Invalid contract type. Choose from vault or trancheToken."
    exit 1
fi